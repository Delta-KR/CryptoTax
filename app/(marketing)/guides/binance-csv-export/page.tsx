...
import { BINANCE_STEPS } from '@/lib/guides/exchange-steps';
import { getGlossaryTerm } from '@/lib/glossary/terms';

const guide = getGuide('binance-csv-export')!;
...
export default function BinanceCsvGuidePage() {
    ...
    <>
      ...
      <section className="section-pad pb-6">
        <div className="mx-auto max-w-content">
          ...
          <SectionEyebrow>BINANCE</SectionEyebrow>
          ...
          <p className="max-w-[600px] text-[16px] leading-[1.7] text-ink-2 text-pretty">
            바이낸스는 거래내역을 CSV로 내보내요. 현물(Spot)만 한국 양도소득세 대상이라, 내보낼 때 계정 유형을 꼭 Spot으로 골라 주세요.
          </p>
        </div>
      </section>
      ...
      <section className="section-pad pt-0">
        <div className="mx-auto flex max-w-content flex-col gap-8">
          ...
          <div className="rounded-md border border-warn/40 bg-warn-soft px-5 py-4 text-[13.5px] leading-[1.65] text-warn">
            <span className="font-semibold">자주 막히는 곳</span> · 한 번에 최대 3개월까지만 받아져요. 1년치는 4번 나눠 받아서 다 올리면 자동으로 합쳐져요. Futures(선물)는 한국 세법상 파생상품 카테고리라 지원하지 않아요 — 계정 유형에서 Spot인지 꼭 확인하세요.
          </div>
