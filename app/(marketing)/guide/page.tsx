...
const UPBIT_STEPS: readonly Step[] = [
  {
    n: 1,
    title: '업비트 웹사이트에 로그인',
    desc: 'upbit.com에 접속하여 본인 계정으로 로그인합니다. 모바일 앱은 PDF 출력이 제한되므로 PC 환경을 권장합니다.',
  },
  {
    n: 2,
    title: '거래내역 페이지로 이동',
    desc: '상단 메뉴 [내정보] → [거래내역]을 클릭합니다. 또는 주소창에 upbit.com/investments/history 를 입력해서 바로 이동할 수도 있습니다.',
  },
  {
    n: 3,
    title: '양도소득 탭 선택',
    desc: '거래내역 화면 상단의 [양도소득] 또는 [전체 내역] 탭을 클릭합니다.',
  },
  {
    n: 4,
    title: '기간 설정',
    desc: '조회하고자 하는 기간(연도별 또는 전체)을 선택합니다. 보통 신고 대상 연도 전체를 선택합니다.',
  },
  {
    n: 5,
    title: 'PDF 출력',
    desc: '화면 우측 또는 하단의 [PDF 출력] 버튼을 클릭하면 .pdf 파일이 자동으로 다운로드됩니다.',
  },
  {
    n: 6,
    title: 'Kontaxt에 업로드',
...
const BINANCE_STEPS: readonly Step[] = [
  {
    n: 1,
    title: 'Binance에 로그인',
    desc: 'binance.com에 접속하여 로그인합니다. 2FA가 설정되어 있다면 인증을 완료해주세요.',
  },
  {
    n: 2,
    title: 'Wallet 메뉴 진입',
    desc: '우측 상단 [Wallet] → [Transaction History] 또는 한국어 환경에서는 [지갑] → [거래 내역]을 클릭합니다.',
  },
  {
    n: 3,
    title: 'Export 버튼 클릭',
    desc: '거래 내역 페이지 우측 상단의 [Export Transaction Records] 또는 [내보내기] 버튼을 누릅니다.',
  },
  {
    n: 4,
    title: 'Spot(현물) 선택 — 중요',
    desc: 'Account type에서 반드시 [Spot]을 선택합니다. Futures(선물)는 현재 한국 세법상 별도 카테고리로 미지원입니다.',
  },
  {
    n: 5,
    title: '기간 + CSV 형식',
    desc: '한 번에 최대 3개월씩 조회 가능합니다. 1년치가 필요하면 4번 나눠 받으세요. 파일 형식은 CSV로 선택합니다.',
  },
  {
    n: 6,
    title: '제출 후 이메일 또는 다운로드 페이지 확인',
    desc: 'Submit 후 처리가 완료되면 가입 이메일로 다운로드 링크가 발송됩니다. 또는 Binance의 [Generated Reports] 페이지에서 직접 받을 수 있습니다.',
  },
  {
    n: 7,
    title: 'Kontaxt에 업로드',
...
const FAQ: ReadonlyArray<{ q: string; a: string }> = [
  {
    q: '제 거래내역 데이터는 안전한가요?',
    a: '업로드된 파일은 서버 메모리에서만 처리되며 디스크에 저장되지 않습니다. 처리가 끝나면 즉시 폐기됩니다. 계산 결과는 본인 브라우저(localStorage)에만 저장되며, 서버나 외부로 전송되지 않습니다.',
  },
  {
    q: '빗썸 거래내역은 어떻게 업로드하나요?',
    a: '현재 빗썸은 거래내역 추출 방식이 마련되지 않아 미지원 상태입니다. 빗썸 고객센터에 직접 거래내역 CSV를 요청하면 받을 수 있지만, 본 서비스에서는 차후 정식 지원 시 자동 통합될 예정입니다.',
  },
  {
    q: '의제취득가액이 뭔가요?',
  ...
  },
  {
    ...
    a: '한국 원화(KRW)가 아닌 통화(USDT, USD 등)로 이뤄진 거래는 거래 시점의 환율로 자동 환산됩니다. 바이낸스 BTC/USDT 거래라면 그 시각의 USDT/KRW 환율을 적용해 KRW 기준 손익을 산출합니다.',
  },
  {
    q: '코인 간 교환(SWAP)은 어떻게 처리되나요?',
    a: '예를 들어 BTC를 ETH로 교환하면 "BTC 매도 + ETH 매수" 2건의 거래로 자동 분리되어 양쪽 모두 총평균 단가 산정에 반영됩니다.',
  },
  {
    q: '선물(Futures)은 지원하나요?',
    a: '현재는 현물(Spot)만 지원합니다. 한국 세법상 가상자산 양도소득세는 현물 거래에 적용되며, 선물은 별도 카테고리(파생상품)에 해당하기 때문입니다.',
  },
  {
    q: '여러 거래소를 함께 처리할 수 있나요?',
    a: '네. 각 탭에 해당 거래소 파일을 차례로 업로드하면 모든 거래가 자동으로 통합·정렬되어 한 화면에서 계산됩니다.',
  },
  {
    q: '계산 결과를 다시 보려면 어떻게 하나요?',
    a: '동일 브라우저에서 다시 접속하면 마지막 계산 결과가 그대로 유지됩니다. 다른 기기·브라우저에서는 파일을 다시 업로드해야 합니다 (현재는 로그인·서버 저장 기능 준비 중).',
  },
];
...
const FLOW_STEPS: readonly FlowItem[] = [
  ...
  },
  {
    n: 3,
    title: '총평균법 + 세법 적용',
    desc: '거주자 시행령 §88①에 따라 연 단위 총평균 단가로 취득가액을 산출하고, 의제취득가액 등 한국 세법 규칙을 자동 적용합니다.',
  },
  {
    n: 4,
    title: '결과 + PDF 리포트',
        ...
        <div className="mx-auto max-w-content">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <ExchangeGuideCard
              ...
              note={{
                tone: 'info',
                text: 'PDF는 한 번에 전체 거래를 받을 수 있어 가장 간편합니다. 매수·매도·입출금 내역이 한 파일에 포함됩니다.',
              }}
            />
            <ExchangeGuideCard
              name="바이낸스"
