...
## 0. 한 줄 디렉션 — **Tier 3 톤 mix = Kontaxt 브랜드 보이스 (2026-05-25 확정)**

> **차분한 정확함 + 친근한 안내. 한국어 우선, 컨텍스트별 톤 mix, 마침표로 닫는다.**

**Tier 3 톤 mix 가 프로젝트 전체에 걸쳐 Kontaxt 의 브랜드 보이스로 확정** (2026-05-25 PR #56·#57·#58·#59·#60·#61·#62·#63·#64 + 이번 #65 누적 9 PR). 마케팅·랜딩 페이지부터 인앱 UI·인증 흐름·이메일까지 모든 사용자-노출 텍스트에 적용.

### 친밀 영역 (해요체) — 안내·약속

- 마케팅 (Hero·CTA·Section·Problem·HowItWorks·Example·Features·Pricing·Roadmap·Footer description)
- Guide / Sample / FAQ
- **(marketing)/simulator**
- **(app)/* 인앱 UI 전체** (Dashboard·Tax·Transactions·Settings·Report·Billing 안내·toast·empty state)
- **(auth)/* 인증 페이지** (login·signup·forgot/reset password 안내·error)
- Toast / Notification / Error message / Empty state

### 격식 영역 (-습니다) — 신뢰·법적 안전망

- **Security 카드 body** — "폐기합니다", "조회·수정 가능합니다" 등
- **Footer 법적 disclaimer** ([footer.tsx:93](components/sections/footer.tsx:93))
- **Email H1** — "확인해 주세요." / "환영합니다." (Email body 는 친밀 mix)
- **Legal** (privacy·terms) — Q6 결정
- **Report 법적 disclaimer** ([report/page.tsx:293](app/(app)/report/page.tsx:293)) — Footer disclaimer 와 같은 패턴
- **Example 법조항 인용** (시행령 §88①·§92②4호 정확 인용 — [example.tsx:184](components/sections/example.tsx:184))

### 광고 puffing — 0건 유지 (§9 안티패턴 29개)

친밀 톤이라도 "혁신적인·압도적·완벽한·역사적 전환점" 같은 puffing 0건. humanizer + VOICE.md §9 grep 3종 통과.
...
이전 표준 (격식 -습니다 통일, 무인칭만) 은 토스·카카오뱅크 대비 "soulless AI 톤" 으로 인식되어 Tier 3 톤 mix 로 전환 → **2026-05-25 정식 확정**.
...
| **Toast** | 친밀 | 1문장, 10~40자 | -됐어요 / -완료 | 사실 | "리포트가 다운로드됐어요." ([report:200](app/(app)/report/page.tsx:200)) |
| **(marketing)/simulator** | 친밀 | 페이지·form·toast 전반 | -해요 / -돼요 / -이에요 | 무인칭 | "5월엔 PDF 한 장이면 돼요" 같은 친밀. 평서체 -다 (-한다·-된다) 금지. ([simulator/page.tsx:39](app/(marketing)/simulator/page.tsx:39)) |
| **(app)/* 인앱 UI (Dashboard·Tax·Transactions·Settings·Report·Billing)** | 친밀 | description·toast·empty state·error | -해요 / -돼요 / -이에요 | 무인칭 / Kontaxt | "{year}년 거래가 없어요." ([dashboard:314](app/(app)/dashboard/page.tsx:314)) · "변경됐어요. 다음 업로드부터 적용돼요." ([tax/settings:96](app/(app)/tax/settings/page.tsx:96)) |
| **(auth)/* 인증 페이지 (login·signup·password)** | 친밀 | error·subtitle·안내 | -해요 / -돼요 / -이에요 | 무인칭 | "비밀번호가 일치하지 않아요." · "재설정 링크를 보내드려요." |
| **Report 법적 disclaimer** | **격식 (절대)** | 1문장, 30~80자 | -합니다 | 무인칭 | "본 리포트는 세무 신고의 참고 자료를 제공하며, 최종 신고는 세무사 검토를 권장합니다." ([report:293](app/(app)/report/page.tsx:293)) |
| **Example 법조항 인용** | **격식 (절대)** | 법조항 인용 — 문어체 OK | -합니다 / 명사형 | 무인칭 | "...총평균법(과세기간 개시일 보유분 + 연내 매수분 합산 ÷ 총수량)으로 산출합니다." ([example:184](components/sections/example.tsx:184)) — 시행령 §88①·§92②4호 정확 인용 |
...
| Q2 | **해요체 (-있어요 / -할게요) 사용** | **(c) 컨텍스트별 mix — Tier 3 = Kontaxt 브랜드 보이스 (2026-05-25 확정, PR #56~#65)**. 친밀 영역 (해요체): 마케팅·Guide·FAQ·Sample·(marketing)/simulator·**(app)/* 인앱 UI**·**(auth)/* 인증**·Toast·Error·Empty state. 격식 절대 유지 (-습니다): **5개 영역** — Security 카드 body·Footer 법적 disclaimer·Email H1·Legal (privacy/terms)·**Report 법적 disclaimer**·**Example 법조항 인용**. 한국 핀테크 reference (토스·카카오뱅크) 친밀 톤 차용으로 "soulless AI 톤" 해소. 자세한 매트릭스는 §3·§10. |
...
- **2026-05-25 (Tier 3 톤 mix = Kontaxt 브랜드 보이스 확정)** — 프로젝트 전체 적용 + 영역 명문화.
  - **확정 선언**: Tier 3 톤 mix 가 Kontaxt 의 공식 브랜드 보이스로 확정. 마케팅·랜딩부터 인앱 UI·인증·이메일까지 모든 사용자-노출 텍스트가 이 표준을 따른다.
  - **누적 9 PR**: #56 신설 → #57 drift fix → #58 humanizer 통합 → #59 무인칭 → #60 Tier 3 mix → #61·#62 orphan wraps → #63 Hero 한 줄 → #64 simulator+app pages → **#65 brand voice 확정 + 잔여 정리**.
  - **#65 변경**: 격식 잔재 50+ 위치 일괄 친밀화 — (app)/* 인앱 UI · (auth)/* 인증 페이지 · components/ui · app/error.tsx · how-it-works step desc 친밀 전환. FileDrop 에러 메시지 "파일이 너무 큽니다" → "너무 커요". sed batch + 수동 Edit 결합.
  - **§0 갱신** — Tier 3 = Kontaxt 브랜드 보이스 확정 선언 + 친밀 영역 / 격식 절대 유지 영역 / 광고 puffing 0건 유지 명문화.
  - **§10 매트릭스 보강** — 4 신규 행 추가: `(marketing)/simulator` · `(app)/* 인앱 UI` · `(auth)/* 인증` · `Report 법적 disclaimer` · `Example 법조항 인용`.
  - **§13 Q2 결정 본문** — 격식 절대 유지 영역 4 → **6 영역** 확장 (Report disclaimer + Example 법조항 신규 명시).
  - **격식 절대 유지 영역 (6)**: Security 카드 body / Footer 법적 disclaimer / Email H1 / Legal (privacy·terms) / Report 법적 disclaimer / Example 법조항 인용.
