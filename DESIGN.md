...
언어 표준은 별도 문서 [VOICE.md](VOICE.md)에 분리되어 있습니다. 페르소나, 종결어 표준, 컨텍스트별 톤 매트릭스(28개), 언어 차원 AI 안티패턴(12개), 어휘 금지·선호 사전, 작성 체크리스트가 모두 거기 있습니다.

핵심 5개만 여기에:

1. **마침표 있는 단문 헤드라인** — "가상자산 양도세, 정확하게."
2. **격식체 -습니다 / -입니다 통일** — 해요체(-있어요/-할게요) 남발 금지. mid-page micro-copy 1줄 예외만.
3. **무인칭 화자 기본** — "Kontaxt는 ...입니다" 같은 메타 자기소개를 본문에 깔지 않는다. 행위 주체로만 "Kontaxt가".
4. **숫자는 한국식 만 단위 + ₩** — `1,550만` / `₩49,900`. K·M 약식 금지.
5. **이모지·느낌표(!) 금지** — 강조는 마침표 + 명사형으로.

새 카피·기존 카피 수정 시 반드시 [VOICE.md](VOICE.md) §12 작성 체크리스트 실행.
