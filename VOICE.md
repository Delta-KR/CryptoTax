...
**2인칭 "당신" 사용 금지** (2026-05-25 Q3 정책 갱신). 무인칭이 표준이고, 소유·접근을 명시해야 할 때는 **"본인"** 을 쓴다.

| 형태 | 예시 | 판정 |
|------|------|------|
| 광고 소유격 "당신의 X" | "당신의 데이터는 어떻게 보호되나요?" | ❌ — Kontaxt가 사용자를 칭하며 묻는 광고 톤 (humanizer §9-B H17 참조) |
| 직접 호명 "당신은 ~세요" | "당신은 파일만 올리세요." | ❌ — 명령형은 주어 생략이 자연스러움 |
| 무인칭 | "데이터는 어떻게 보호되나요?" / "파일만 올리세요." | ✅ |
| "본인" (소유·접근 명시) | "본인 데이터만 접근" ([security.tsx:37](components/sections/security.tsx:37)) · "본인 데이터 격리 (RLS)" ([trust-strip:33](components/sections/trust-strip.tsx:33)) · "본인 데이터로 동일한 리포트" ([sample:53](app/(marketing)/sample/page.tsx:53)) | ✅ — 코드베이스 표준 패턴 |

**핵심**: 같은 컴포넌트 안에서도 일관성 — `security.tsx` 카드 제목이 "본인 데이터만 접근" 인데 SectionTitle 만 "당신의 데이터" 였던 게 Q3 정책 (b) → (d) 갱신 계기.
...
| **Section Title** | 8~16자, 1문장 | 마침표·물음표 (2026-05-25 Q1 결정) | `,` `.` `?` `<br/>` | **무인칭 / "본인" / 의문** — 소유격 "당신의 X" 금지 (Q3 갱신) | "한국 세법, 빠짐없이 반영." ([features:161](components/sections/features.tsx:161)) · "데이터는<br/>어떻게 보호되나요?" ([security:104](components/sections/security.tsx:104)) |
...
| Q3 | **2인칭 "당신" 사용 빈도** | **(d) 사용 금지, 무인칭/본인 통일** (2026-05-25 갱신, 이전 결정 (b) "절제 사용" 폐기) — 의문문+소유격 결합("당신의 X는 어떻게 ~나요?")이 광고 톤이라 §9-B H17 신규 안티패턴. 무인칭 표준, 소유·접근 명시 시 "본인" (코드베이스 표준 — trust-strip:33 · security:37 · sample:53). 현재 2건(security:104, how-it-works:135) 모두 fix. |
