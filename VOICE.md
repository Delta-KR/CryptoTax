...
> **차분한 정확함. 한국어로 우선, 격식체로, 단문으로, 마침표로 닫는다.**

타깃은 한국 가상자산 투자자 — 전문가도 회계사도 아닌 일반 보유자. 신뢰·정확함·차분함이 1순위. 화려함·과장·친근체 남발은 0순위.

레퍼런스 (DESIGN.md §1과 동일):
- **Mercury** — 라이트 미니멀 핀테크 톤
- **Obsidian** — 에디토리얼 단문 헤드라인 (마침표 사용)
- **Rogo** — 컴플라이언스 신뢰 시그널
...
1. **에디토리얼 단문 헤드라인** — 한 문장. 마침표로 닫는다. "가상자산 양도세, 정확하게."
2. **격식체로 차분하게** — 본문은 -습니다/-입니다. 친근체(-해요/-있어요)는 mid-page CTA 한 줄에 한해 예외 허용. 남발 금지.
3. **출처가 보이는 신뢰** — 근거 있는 숫자·법조항(시행령 §88①)·기관명 인용. 막연한 권위 금지 ("최고의", "압도적인").
4. **자기소개 자제** — "Kontaxt는 ... 서비스입니다" 같은 메타-설명을 본문에 깔지 않는다. 화자는 무인칭이 기본, "Kontaxt가" 는 행위 주체로만.
5. **언어 차원의 AI 티 제거** — DESIGN.md §8이 시각 안티패턴을 다룬다면, 이 문서 §9가 언어 안티패턴을 다룬다. ChatGPT 스러운 한국어 패턴은 시각 글로우만큼 신뢰를 깎는다.
...
**2인칭 "당신"은 절제 사용.** 현재 코드베이스 등장:
- [security.tsx:104](components/sections/security.tsx:104) — "당신의 데이터는 어떻게 보호되나요"
- [how-it-works.tsx:135](components/sections/how-it-works.tsx:135) — "당신은 파일만 올리세요"

**규칙**: "당신"은 **사용자와 직접 대화하는 한 줄**(보안 약속, 사용 안내)에서만 1~2회. 매 섹션마다 "당신"이 등장하면 광고 톤이 된다.
...
본문·헤드라인·FAQ·이메일·법률 모두 **-습니다 / -입니다 / -합니다** 계열을 기본으로 한다.

- "만들어드립니다" ([hero.tsx:268](components/sections/hero.tsx:268))
- "받지 않습니다" ([security.tsx:109](components/sections/security.tsx:109))
- "처음부터 받지 않습니다" ([security.tsx:109](components/sections/security.tsx:109))
- "추적합니다" ([features.tsx:19](components/sections/features.tsx:19))
- "끝납니다" ([how-it-works.tsx:131](components/sections/how-it-works.tsx:131))
- "재설정해 주세요" ([reset-password.tsx:31](emails/reset-password.tsx:31))

### 명령형 — -세요 / -해 주세요

CTA 버튼, 안내문, 직접 행동 요청에 한정:
- "지금 무료로 시작하세요" ([cta.tsx:53-55](components/sections/cta.tsx:53))
- "당신은 파일만 올리세요" ([how-it-works.tsx:135](components/sections/how-it-works.tsx:135))
- "보세요" ([example.tsx:171](components/sections/example.tsx:171))
- "확인해 주세요" ([verify-email.tsx:33](emails/verify-email.tsx:33))

### 명사형 단정 — 헤드라인·라벨·trust strip

부드러운 마침표 단문에 어울리는 명사형 종결:
- "정확하게." ([hero.tsx:265](components/sections/hero.tsx:265))
...
### 해요체 — 사용 금지 (2026-05-25 Q2 결정)

**-해요 / -예요 / -있어요 / -할게요는 모든 컨텍스트에서 사용하지 않는다.** 본문·헤드라인·CTA·법률·security·이메일·disclaimer 전부 -습니다 / -입니다로 통일.

현재 코드에서 발견된 드리프트 (drift fix 대상):
- [pricing.tsx:149](components/sections/pricing.tsx:149) — "확인할 수 있어요" → **"확인할 수 있습니다."**
- [roadmap.tsx:117](components/sections/roadmap.tsx:117) — "보여드릴게요" → **"미리 안내합니다."** 또는 **"단계별로 공개합니다."**
- [roadmap.tsx:128](components/sections/roadmap.tsx:128) — "조정될 수 있어요" → **"조정될 수 있습니다."**
- [exchanges.tsx:156-158](components/sections/exchanges.tsx:155) — "알려주세요 · 평균 2주 내 추가" — "알려주세요"는 명령형 -세요(해요체 아님). **유지**.

### 헤지 vs 단정

- **사실은 단정**: "폐기됩니다", "저장됩니다", "발생합니다"
- **옵션·선택지·가능성은 헤지**: "~할 수 있습니다", "~될 수 있습니다"
- **약속은 단정 + 근거**: "회원은 본인 데이터만 접근 가능합니다." (RLS 근거)

**금지**: 사실 단정에 헤지 끼우기 — "처리될 수 있습니다" (사실 처리됨) → "처리됩니다".
...
| `1,550만` | 사용자 수 ([cta:88](components/sections/cta.tsx:88)) — 단, README:232에 "1,150~1,250만"으로 다르게 표기됨 → **README sync 필요** |
| `247건` | 거래 건수 |
...
DESIGN.md §8이 시각 안티패턴 10가지를 정의한다면, 이 절은 **언어 안티패턴 12가지**를 정의한다. 새 카피 작성·리뷰 시 이 패턴이 나타나면 즉시 교체.
...
**언어 안티패턴 검출 명령** (수동 grep):
```bash
grep -rEn "여러분|고객님|솔루션|혁신|압도적|완벽한|최고의|강력한|단 한 번|이제 더 이상|걱정 없이|차원이 다른|꿀팁|꿀템|찐텐|!" \
...
각 사용자-노출 컨텍스트의 표준 형식·길이·종결·예시.

| 컨텍스트 | 길이 | 종결 | 허용 구두점 | 화자 | 모범 사례 |
|----------|------|------|-------------|------|-----------|
| **Hero H1** | 8~14자, 1문장, `<br/>` OK | 마침표 또는 명사형 | `,` `.` `<br/>` | 무인칭 | "가상자산 양도세,<br/>정확하게." ([hero:263](components/sections/hero.tsx:263)) |
| **Hero Subhead** | 1문장, 30~60자 | -습니다 | `,` `.` | 무인칭 / "Kontaxt가" | "거래소 데이터를 통합해 한국 세법 기준으로 자동 계산하고, 세무사 전달용 PDF까지 만들어드립니다." ([hero:268](components/sections/hero.tsx:267)) |
| **Hero CTA 버튼** | 5~10자 | -하기 / -받기 | (없음) | 사용자 행동 | "무료로 시작하기" ([hero:276](components/sections/hero.tsx:276)), "샘플 리포트 보기" ([hero:300](components/sections/hero.tsx:300)) |
| **Hero Check chip** | 5~12자 | 명사형 | (없음) | 사실 | "신용카드 불필요" ([hero:305](components/sections/hero.tsx:305)), "결제 전 미리보기 무료", "2분만에 첫 결과" |
| **Section Eyebrow** | 영문 UPPERCASE 4~16자 | (없음) | (없음) | 카테고리 | "SECURITY", "PRICING", "HOW IT WORKS", "USER GUIDE" |
| **Section Title** | 8~16자, 1문장 | 마침표·물음표 표준 (현재 거의 누락 — 수정 필요) | `,` `.` `?` `<br/>` | 무인칭 / 의문 | (권장) "한국 세법, 빠짐없이 반영합니다." / (현재) "한국 세법, 빠짐없이 반영" |
| **Section Lead** | 1~2문장, 35~70자 | -습니다 | `,` `.` `·` `—` | 무인칭 | "한국 핀테크 표준을 기본으로, 보관하지 않을 데이터는 처음부터 받지 않습니다." ([security:108](components/sections/security.tsx:108)) |
| **Card Title (h3)** | 4~12자 | 명사형 또는 마침표 없음 | (없음) | 사실 | "거래내역 비저장", "다중 거래소 데이터 통합" |
| **Card Body (desc)** | 1~2문장, 30~80자 | -습니다 | `,` `.` `·` | 무인칭 / Kontaxt | "업로드한 거래내역 파일은 계산 직후 폐기합니다. 계산 결과는 본인 브라우저에만 저장됩니다." ([security:17](components/sections/security.tsx:17)) |
| **Button Primary** | 5~10자 | -하기 / -세요 | (없음) | 행동 | "무료로 시작하기", "출시 알림 받기" |
| **Button Secondary** | 5~12자 | -보기 / -이동 / 명사 | (없음) | 행동 | "샘플 리포트 보기", "요금제 비교" |
| **Trust strip label** | 4~12자 | 명사형 | (없음) | 사실 | "한국 PIPA 준수", "Cloudflare 봇 차단", "본인 데이터 격리 (RLS)" |
| **Pricing tier name** | 1~6자 | 명사 | (없음) | 카테고리 | "무료", "구독", "원타임" |
| **Pricing tier tag** | 4~10자 | 명사 | `·` | 분류 | "체험·검증", "연중 절세 도구", "한 해만 신고" |
| **Pricing tier desc** | 1문장, 25~60자 | -습니다 / -에게. | `,` `.` | 무인칭 | "결제 전 결과를 미리 확인할 수 있습니다." ([pricing:28](components/sections/pricing.tsx:28)) |
| **FAQ Q** | 6~20자, 1문장 | 물음표 | `?` | 사용자 발화 | "제 거래내역 데이터는 안전한가요?" ([guide:93](app/(marketing)/guide/page.tsx:93)) |
| **FAQ A** | 1~3문장, 60~150자 | -습니다 | `,` `.` `·` | 무인칭 / Kontaxt | "업로드된 파일은 서버 메모리에서만 처리되며 디스크에 저장되지 않습니다. ..." ([guide:94](app/(marketing)/guide/page.tsx:94)) |
| **Footer link** | 3~10자 | 명사 | (없음) | 카테고리 | "보안 개요", "취약점 신고", "이용약관" ([footer:68-87](components/sections/footer.tsx:68)) |
| **Footer fine-print** | 1문장, 30~80자 | -합니다 | `,` `.` | 무인칭 | "본 서비스는 세무 신고의 참고 자료를 제공하며, 최종 신고는 세무사 검토를 권장합니다." ([footer:93](components/sections/footer.tsx:93)) |
| **Footer disclaimer 작은 라인** | 1문장 | -습니다 | `·` `.` | 카테고리 | (수정 예정) "일정은 진행 상황에 따라 조정될 수 있습니다 · 본 정보는 세무 조언이 아닙니다." — 현재 [roadmap:128](components/sections/roadmap.tsx:128)이 "있어요" drift 상태, Q2 결정에 따라 격식 통일 |
| **Email Subject (Preview)** | 1문장, 20~40자 | -습니다 / -주세요 | `,` `.` | 무인칭 | "이메일 주소를 확인해 주세요. 60분 안에 인증해주세요." ([verify-email.tsx:29](emails/verify-email.tsx:29)) |
| **Email Eyebrow** | 영문 UPPERCASE | (없음) | (없음) | 카테고리 | "EMAIL VERIFICATION", "PASSWORD RESET", "WELCOME" |
| **Email H1** | 1문장, 8~20자 | **마침표 필수** | `,` `.` | 무인칭 / Kontaxt | "이메일 주소를 확인해 주세요." / "Kontaxt에 오신 것을 환영합니다." |
| **Email subtitleEn** | 1문장 영문 | 마침표 | `.` | 무인칭 | "Please verify your email." / "Welcome to Kontaxt." |
| **Email Body** | 1~2 paragraph, 1문장씩 | -습니다 / -주세요 | `,` `.` | 무인칭 | "아래 버튼을 눌러 ... 주소가 본인 것인지 확인해 주세요." |
| **Email CTA 버튼** | 4~10자 | -하기 / 명사 | (없음) | 행동 | "이메일 인증하기", "비밀번호 재설정", "대시보드로 이동" |
| **Email footer** | 1문장 | 명사형 | `·` | 카테고리 | "Kontaxt · 가상자산 양도세 정산 · 발신 전용" ([EmailLayout:158](emails/components/EmailLayout.tsx:158)) |
| **Legal H1** | 4~10자 | (없음) | (없음) | 카테고리 | "이용약관", "개인정보처리방침" |
| **Legal Article 제목** | 4~14자 | (없음) | (없음) | 카테고리 | "정의", "환불 정책", "서비스의 책임 제한" |
| **Legal Clause** | 1문장 길게 OK | -합니다 | `,` `.` `「」` | "서비스" / "회원" | 격식 법률문 형식. 한 호 안에서 의미 단위 줄바꿈 OK |
| **Error message** | 1문장, 20~50자 | -습니다 / -주세요 | `,` `.` | 무인칭 | "이메일 주소를 다시 확인해 주세요." (가이드 제안) |
| **Empty state** | 1~2문장 | -습니다 | `,` `.` | 무인칭 | "아직 업로드한 거래내역이 없습니다. 거래소 파일을 끌어다 놓으세요." (가이드 제안) |
| **Toast** | 1문장, 10~40자 | -습니다 / 명사형 | `,` `.` | 사실 | "리포트가 다운로드되었습니다." (가이드 제안) |

---

## 11. We Are / We Are Not

| ✅ We Are | ❌ We Are Not |
|-----------|--------------|
| 차분한 정확함 | 들뜬 자신감 |
| 한국어 우선, 영어는 보조 | 한·영 혼용 마케팅 직역 |
| 격식체 (-합니다 / -입니다) | 친근체 (-해요 / -있어요) 남발 |
| 단문 + 마침표 헤드라인 | 마침표 없는 슬로건 (현재 위반 多, 수정 진행) |
| 출처·근거 명시 (시행령 §88①, RLS, PIPA) | 막연한 권위 ("업계 최고") |
| 무인칭 화자, 행위 주체로만 "Kontaxt가" | "Kontaxt는 ... 입니다" 자기소개 본문 |
| 자동·정확·즉시·한 번에 | 혁신·완벽·최고·압도적 |
| 숫자 풀어쓰기 (1,550만 / ₩49,900) | K·M 약식 (1.5M / $50K) |
| 한국 세법 용어 풀어쓰기 (총평균법, 의제취득가액) | 영문 마케팅 용어 그대로 (Tax-Loss Harvesting 단독) |
| 거래소 quirks를 사실로 서술 | 경쟁사·거래소 비교에서 결론 띄우기 |
| CTA 단계적 노출 (Hero → mid → 마지막) | "지금 바로!" 강압 + 느낌표 |
| 보안·컴플라이언스는 별도 섹션으로 시각화 | 약관 footnote 한 줄로 묻기 |
...
- [ ] 격식체 (-합니다)로 통일되었는가 (예외: mid-page micro-copy 1줄)
...
**작성 후 — Soft 금지 grep**:
```bash
grep -En "솔루션|스마트한|원스톱|초간단|초고속|혁신|차원이 다른|걱정 없이|고민 끝|꿀팁|꿀템" <new-file>
```

**작성 후 — Hard 금지 grep**:
```bash
grep -En "고객님|여러분|단 한 번의 클릭|이제 더 이상|~에 지치셨나요|!" <new-file>
```

**작성 후 — AI 패턴 grep**:
```bash
grep -En "강력하고|정확하며|신뢰할 수 있는|완벽한|최고의|압도적|매우|굉장히|정말로" <new-file>
```
...
| Q2 | **해요체 (-있어요 / -할게요) 사용** | **(a) 격식 -습니다 통일** — pricing.tsx:149, roadmap.tsx:117, roadmap.tsx:128 3건 모두 -습니다로 변경. mid-page CTA·disclaimer 예외 없음. |
| Q3 | **2인칭 "당신" 사용 빈도** | **(b) 보안·안내 한 줄에만 절제 사용** — 현재 2건 (security:104, how-it-works:135) 유지. 새 카피에 무분별 추가 금지. |
...
- **2026-05-25** — 초안 작성 + 정책 9개 확정 (Q1~Q9).
  ...
  - **남은 작업**: drift fix PR (Section Title 마침표 10개 · 해요체 3건 · 이모지 1건 · Tax-Loss Harvesting 병기 · README 1,550만 sync) — 별도 PR로 진행
