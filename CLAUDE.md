...
**Kontaxt** — 한국 가상자산 양도소득세(2027.01.01 시행) 정산 플랫폼. Upbit / Bithumb / Binance CSV·PDF·XLS를 한국 세법(거주자 **총평균법** — 소득세법 시행령 §88①·§92②4호 2025-02-28 개정, 의제취득가액 §88②, 필요경비 의제율 50% §88⑤, 250만원 공제, **분리과세 기타소득 20% + 지방세 2%**)에 맞춰 통합 계산. FIFO·이동평균은 비거주자 모드(§183⑥) 또는 참고용으로만 보존.
...
### 4b) PR 머지·작업 완료 시 vault·repo TODO 즉시 sync ([[feedback_auto_todo_sync]])

PR 머지·기능 완료·검증 완료 직후 같은 turn 안에 **자동으로** TODO back-update. 다음 세션 / "이거 다 미완료야?" 같은 질문 나오기 전에. 2026-05-28 Cowork 157건 audit 에서 56건이 back-update 미실행이었던 사고로 routine 격상.

루틴:
1. `gh pr view <PR>` 으로 title·body 확보
2. vault `Daily/2026-MM-DD.md` (오늘 + 직전 1~2일) + `Projects/*.md` + repo `TASKS.md` 에 PR 키워드 grep
3. 매칭 `- [ ]` → `- [x]` + PR 링크·commit·완료 시점 한 줄 덧붙임
4. carry 항목이면 **이전 노트도 거꾸로 [x]** ("→ 다음날 carry" 라고만 적는 패턴 금지)
5. 폐기 결정은 `[x] ~~취소선~~ — 폐기 사유` 패턴 (그대로 [ ] 두면 미완료 카운트)
6. 세션 종료 시 (Stop) 이번 세션 만진 항목 전부 vault·repo cross-check, 빠진 [x] 마무리 전 처리

검증 strict — 확신 없는 항목은 `[~]` 부분 또는 `[ ]` 유지. 잘못 [x] 치면 다음 audit 때 더 큰 혼란.
