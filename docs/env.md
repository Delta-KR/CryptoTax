...
확정 PG = **포트원 (PortOne)**. 카카오페이·네이버페이·토스페이·신용카드 일괄 통합 (`pricing-strategy-summary.md` §6.5). v2 후보: 토스페이먼츠 직접 연동 (안정성·공식성 필요 시점).

포트원 통합 시 추가될 env vars (현재는 미사용, 정확한 키 이름은 가입 후 확정):

| 키 | 용도 |
|---|---|
| `PORTONE_API_SECRET` | 결제 승인 API 호출 |
